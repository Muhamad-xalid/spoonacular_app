...
export default function Home() {
  ...
  return (
    <div className={`w-full h-3/4 ${darkmode && 'dark:bg-black'}`}>
      <h1 className={`text-center pt-5 text-2xl font-bold ${darkmode && 'dark:text-white'}`}>Spoonacular</h1>
      <div className='container mx-auto pt-20 pb-48 flex justify-between  '>
          <div className='flex flex-col justify-between'>
              ...
              <div>
                <a className={`cursor-pointer p-3 border border-red-900 font-serif rounded-lg mx-2 ${darkmode && 'dark:text-white'}`} onClick={()=>toOurProducts()}>Our Products</a>
                <a className={`cursor-pointer p-3 border border-red-900 font-serif rounded-lg mx-2 ${darkmode && 'dark:text-white'}`} onClick={()=>toOurMeals()}>Our Meal</a>
